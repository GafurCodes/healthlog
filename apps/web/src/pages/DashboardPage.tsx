...
export const DashboardPage: React.FC = () => {
  ...
  const calorieChartData = Object.entries(caloriesByDay).map(([date, calories]) => ({
    date: format(new Date(date), 'MMM dd'),
    calories,
  }));

  const workoutChartData = Object.entries(workoutDurationByDay).map(([date, duration]) => ({
    date: format(new Date(date), 'MMM dd'),
    duration,
  }));
  ...
  return (
    <div className={styles.container}>
      <div>
        ...
        <p style={{ color: 'var(--color-text-light)', marginBottom: '2rem' }}>Here's your wellness summary</p>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem', marginBottom: '2rem' }}>
          <Card>
            <CardBody>
              <h3 style={{ marginTop: 0 }}>Total Meals</h3>
              <p style={{ fontSize: '2rem', margin: '0.5rem 0' }}>{mealLogs.length}</p>
              <p style={{ color: 'var(--color-text-light)', margin: 0 }}>this period</p>
            </CardBody>
          </Card>
          ...
          <Card>
            <CardBody>
              ...
              <p style={{ fontSize: '2rem', margin: '0.5rem 0' }}>{totalCalories}</p>
              <p style={{ color: 'var(--color-text-light)', margin: 0 }}>kcal</p>
            </CardBody>
          </Card>
          ...
          <Card>
            <CardBody>
              ...
              <p style={{ fontSize: '2rem', margin: '0.5rem 0' }}>{workoutLogs.length}</p>
              <p style={{ color: 'var(--color-text-light)', margin: 0 }}>completed</p>
            </CardBody>
          </Card>
          ...
          <Card>
            <CardBody>
              ...
              <p style={{ color: 'var(--color-text-light)', margin: 0 }}>hours</p>
            </CardBody>
          </Card>
        </div>
            ...
            <CardBody>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1rem' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'flex-end' }}>
                  <Button fullWidth onClick={loadLogs} style={{ marginBottom: 0 }}>
                    Refresh
                  </Button>
                </div>
              </div>
        ...
        {loading ? (
          ...
          <div style={{ display: 'grid', gap: '2rem' }}>
                ...
                <CardBody>
                  <ResponsiveContainer width="100%" height={300}>
                    <LineChart data={workoutChartData}>
                      ...
                      <Tooltip />
                      <Legend />
                      <Line type="monotone" dataKey="duration" stroke="#ff9800" strokeWidth={2} />
                    </LineChart>
                  </ResponsiveContainer>
                </CardBody>
              </Card>
                ...
                <CardBody>
                  <ResponsiveContainer width="100%" height={300}>
                    <LineChart data={sleepChartData}>
                      ...
                      <Tooltip />
                      <Legend />
                      <Line type="monotone" dataKey="hours" stroke="#2196f3" strokeWidth={2} />
                    </LineChart>
                  </ResponsiveContainer>
                </CardBody>
              </Card>
            ...
            {calorieChartData.length === 0 && workoutChartData.length === 0 && sleepChartData.length === 0 && (
              <Card>
                <CardBody>
                  <p className={styles['text-center']}>No data to display for selected period</p>
                </CardBody>
              </Card>
            )}
          </div>
